...
namespace FluiTec.DatevSharp.Validation
{
    public class DatevFileValidator : AbstractValidator<DatevFile>
    {
        public DatevFileValidator()
        {
            ...
            RuleFor(file => file.Rows).Must(rows => rows != null && rows.Count > 0)
                .WithMessage("Exported rows must be > 0!");
            RuleForEach(file => file.Rows).SetValidator(file => file.GetValidator());
        }
    }
}